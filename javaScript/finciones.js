...
const flipCard = card => {
  ...
  if (state.flippedCards === 2) {
    ...
    if (card1.querySelector('.card-back img').src === card2.querySelector('.card-back img').src) {
      ...
      state.totalMatches++;
      selectors.matches.innerText = `${state.totalMatches} ACIERTOS`;
    }
    ...
    setTimeout(() => {
      ...
      selectors.win.innerHTML = `
        <span class="win-text">
          ...
          EN <span class="highlight">${state.totalTime}</span> SEGUNDOS<br />
          Y: <span class="highlight">${state.totalMatches}</span> ACIERTOS
        </span>
      `;
